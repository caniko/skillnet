...
> `ai-skills` repo's `multi-phase-plan` skill to _consume_ the
...
- **`/data/nvme0/can/Projects/skillnet` does not exist yet.** The
  ...
  them; Phases 01–04 are written against the _target_ layout.
...
_changelog_ (the audit trail of threshold edits) lives in
...
- **Phase 01** is the SQLite SPOF for the entire CLI: a wrong
  ...
  06's hook contract.
- **Phase 07** is the _publication_ SPOF: a botched crates.io
...
- Skillnet crate is `MIT` _or_ `Apache-2.0` (pick in Phase 07; default
