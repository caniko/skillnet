...
- Do **not** add new test scenarios beyond what the old test file
  covered, _unless_ they're necessary to validate behavior new to
...
- **Dropping coverage by translating only the happy paths.**
  ...
  "unknown scope" errors are _better_ than the old runtime errors —
  cover them.
...
- **Forgetting `cargo clippy --all-targets` until the end.**
  ...
  `--all-targets` as the _first_ step of this phase, after `cargo
